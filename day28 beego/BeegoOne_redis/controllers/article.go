...
func (this *ArticleController) HandleSelect() {
	...
	*/
	ormObject.QueryTable("Article").RelatedSel("ArticleTYpe").Filter("ArticleType__TypeName", typeName).All(&Articles)
}
...
func (this *ArticleController) ShowArticleList() {
	...
	// 1. 查询操作
	ormObject := orm.NewOrm()
	qs := ormObject.QueryTable("article")
	...
	if typeName == "" {
		...
		qs.Limit(PageSize, PageSize*(pageIndex-1)).RelatedSel("ArticleType").All(&articleWithType)
	} else {
		...
		// 用浮点数相除为了避免出现无法整除半页的情况
		CountPage = float64(count) / float64(PageSize)
		qs.Limit(PageSize, PageSize*(pageIndex-1)).RelatedSel("ArticleType").Filter("ArticleType__TypeName", typeName).All(&articleWithType)
	}

	// 获取类型数据
	articleTypes := make([]models.ArticleType, 0) // 存储所有 type 对象的数组
	...
	if len(articleTypes) == 0 {
		...
		_, err = conn.Do("set", "articleTypes", buffer.Bytes())
		if err != nil {
...
func (this *ArticleController) AddType() {
	...
	// 插入数据操作
	_, err := ormObject.Insert(&articleType)
	if err != nil {
...
func (this *ArticleController) DelArticleType() {
	...
	_, err = ormObject.Delete(&articleType)
	if err != nil {
